...
async function verifyWithGumroad(email, licenseKey) {
  ...
  try {
    ...
    if (typeof window !== "undefined" && window.appGet) {
    ...
    if (alreadyActivated) {
      if (typeof window !== "undefined" && window.appSet)
        await window.appSet({ lastLicenseCheck: Date.now(), lastLicenseAttempt: Date.now() });
      return { valid: true };
    }
...
async function checkLicensePeriodically(email, licenseKey) {
  if (!email || !licenseKey) return null;
  if (typeof window === "undefined" || !window.appGet || !window.appSet) return null;
  try {
    const stored    = await window.appGet(["lastLicenseCheck", "lastLicenseAttempt"]);
    const now       = Date.now();
    ...
    await window.appSet({ lastLicenseAttempt: now });

    const data = await _callGumroad(licenseKey, false);

    // Got a definitive response — reset to full 24-hour cycle.
    await window.appSet({ lastLicenseCheck: now, lastLicenseAttempt: now });
