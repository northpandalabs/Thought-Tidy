...
function initCommonSettingsWiring(s) {
  ...
  document.getElementById("prompt-quick-select")?.addEventListener("change", (e) => {
    ...
    if (addBtn)  { addBtn.dataset.editId = p.id; addBtn.textContent = "Update Action"; addBtn.disabled = false; }
...
async function initHistoryPinSection(s) {
  ...
  if (hasPin) {
    ...
    unlockBtn.id = "history-unlock-btn"; unlockBtn.textContent = "Unlock";
    ...
    if (isPro) {
      ...
      changeForm.style.cssText = "display:none;margin-top:10px;padding:12px;background:var(--bg-main,#181825);border:1px solid var(--border,#313244);border-radius:6px;max-width:360px;display:flex;flex-direction:column;gap:8px";
      ...
      document.getElementById("history-change-pin-btn")?.addEventListener("click", () => {
    ...
    setForm.style.cssText = "display:none;margin-top:10px;padding:12px;background:var(--bg-main,#181825);border:1px solid var(--border,#313244);border-radius:6px;max-width:360px;display:flex;flex-direction:column;gap:8px";
    ...
    const saveBtn = document.createElement("button");
    saveBtn.textContent = "Save Passcode";
...
function initGrammarFiltersSection(s) {
  ...
  title.style.cssText = "margin:0 0 6px 0;font-size:13px;font-weight:700;color:var(--text,#cdd6f4)";
  title.textContent = "AI Grammar Filters";
  ...
  sec.appendChild(_toggle("gf-no-headings", "No section headings (##, bold headers)",          gf.noHeadings));
  sec.appendChild(_toggle("gf-no-filler",   "No filler openers (Additionally, Furthermore…)", gf.noFillerOpeners));
  sec.appendChild(_toggle("gf-no-signoffs", "No formal sign-offs (I hope this helps…)",        gf.noFormalSignoffs));

  const saveRow = document.createElement("div");
  saveRow.style.cssText = "margin-top:14px;display:flex;align-items:center;gap:12px";
  const saveBtn = document.createElement("button");
  saveBtn.id = "grammar-save-btn"; saveBtn.textContent = "Save Grammar Filters";
...
function initExportImportSection(s) {
  ...
  titleEl.style.cssText = "font-size:12px;font-weight:700;color:var(--text-muted,#6c7086);text-transform:uppercase;letter-spacing:0.04em;margin:0 0 10px 0";
  ...
  exportForm.style.cssText = "display:none;margin-top:12px;padding:12px;background:var(--bg-main,#181825);border:1px solid var(--border,#313244);border-radius:6px";
  ...
  exportActRow.style.cssText = "display:flex;gap:8px";
  const doExportBtn = document.createElement("button");
  doExportBtn.id = "do-export-btn"; doExportBtn.textContent = "Export";
  const cancelExportBtn = document.createElement("button");
  cancelExportBtn.textContent = "Cancel";
  cancelExportBtn.style.cssText = "background:none;border:1px solid var(--surface,#313244);color:var(--text-muted,#6c7086)";
  ...
  doExportBtn.addEventListener("click", async () => {
    ...
    const confirm = document.getElementById("export-pin-confirm")?.value || "";
    const errEl   = document.getElementById("export-err");
    errEl.style.display = "none";
    if (!pin || !confirm)  { errEl.textContent = "Enter and confirm a PIN."; errEl.style.display = ""; return; }
    if (pin !== confirm)   { errEl.textContent = "PINs don't match."; errEl.style.display = ""; return; }
    ...
    try {
      ...
      const stored = await window.appGet([
        "configuredProviders", "geminiModels", "openaiKey", "claudeKey", "geminiKey",
        ...
        "grammarFilters", "historyFull", "historyLog"
      ]);
  ...
  importBtn.addEventListener("click", async () => {
    ...
    let content;
    try { content = await window.platformOpenBackup(); } catch { return; }
    if (!content) return;

    let parsed;
    try { parsed = parseExport(content); } catch (err) { alert("Invalid backup file: " + err.message); return; }

    const pin = prompt("Enter the PIN used when this backup was created:");
    if (pin === null) return;

    let decrypted;
    try { decrypted = await decryptExport(parsed, pin); } catch { alert("Incorrect PIN. Import cancelled."); return; }

    if (!confirm("This will overwrite all current settings and history. Continue?")) return;

    if (decrypted.settings) await window.appSet(decrypted.settings);
    ...
    alert("Backup imported successfully. Reload to apply all settings.");
    location.reload();
  });
