...
function renderSetupOverview(s) {
  const el = document.getElementById("setup-overview");
  if (!el) return;
  const providers = s?.configuredProviders || configuredProviders || [];
  const isPro     = isProUnlocked(s || {});
  const LABELS    = { openai: "ChatGPT", claude: "Claude", gemini: "Gemini" };

  el.innerHTML = "";
  el.className = "setup-overview";

  // ── Provider pills ───────────────────────────────────────────────────────────
  const provRow = document.createElement("div");
  provRow.className = "sov-row";

  if (!providers.length) {
    const chip = document.createElement("span");
    chip.className   = "sov-chip sov-chip-warn";
    chip.textContent = "○ No AI provider — add one below";
    provRow.appendChild(chip);
  } else {
    providers.forEach((p, i) => {
      const chip  = document.createElement("span");
      chip.className = "sov-chip sov-chip-ok";
      const label = LABELS[p.id] || p.id;
      const model = p.model ? ` · ${p.model}` : "";
      chip.textContent = (i === 0 ? "● " : "○ ") + label + model;
      if (i === 0) chip.title = "Priority provider";
      provRow.appendChild(chip);
    });
  }
  ...
  badgeRow.className = "sov-badges";

  const proBadge = document.createElement("span");
  proBadge.className   = isPro ? "sov-badge sov-badge-pro" : "sov-badge sov-badge-free";
  proBadge.textContent = isPro ? "⚡ Pro" : "Free tier";
  badgeRow.appendChild(proBadge);

  if (s?.profileEnabled && (s.profileName || s.profileRole)) {
    const profBadge = document.createElement("span");
    profBadge.className   = "sov-badge sov-badge-profile";
    profBadge.textContent = "👤 Profile on";
