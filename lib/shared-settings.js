...
function initProSection() {
  ...
  proKeyShowBtn?.addEventListener("click", () => {
    ...
    const reveal = proKeyInput.type === "password";
    proKeyInput.type = reveal ? "text" : "password";
    proKeyShowBtn.textContent = reveal ? "Hide" : "Show";
  });
  ...
  proKeyInput?.addEventListener("input", (e) => {
    ...
    e.target.setSelectionRange(proKeyReal.length, proKeyReal.length);
  });
