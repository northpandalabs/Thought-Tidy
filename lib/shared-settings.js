...
const PROVIDER_INFO = {
  ...
  ollama:  { name: "Ollama (Local AI)",  sub: "Llama, Mistral, Phi…",      keyPlaceholder: "",         keyUrl: "" },
...
function buildCard(p, idx) {
  ...
  if (p.id === "ollama") {
    ...
    urlInput.placeholder = "http://localhost:11434"; urlInput.autocomplete = "off"; urlInput.maxLength = 512;
...
async function saveCardEdit(card, idx, provider) {
  if (provider.id === "ollama") {
    const newUrl   = sanitizeText(card.querySelector(".pc-ollama-url-input")?.value ?? "") || "http://localhost:11434";
    const sel      = card.querySelector(".pc-model-select");
...
async function saveWizardProvider() {
  ...
  if (wizardProvider === "ollama") {
    ...
    const baseUrl = sanitizeText(document.getElementById("wizard-ollama-url")?.value ?? "") || "http://localhost:11434";
...
function initProSection() {
  ...
  proKeyInput?.addEventListener("paste", (e) => {
    ...
    if (!pasted) return;
    const raw = (proKeyReal.replace(/-/g, "") + pasted).slice(0, 32);
