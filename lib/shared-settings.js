...
function initProSection() {
  ...
  // Show real formatted key while editing, mask on blur
  proKeyInput?.addEventListener("focus", () => {
    if (proKeyInput) proKeyInput.value = proKeyReal;
  });
  proKeyInput?.addEventListener("blur", () => {
    if (proKeyMasked && proKeyReal && proKeyInput) proKeyInput.value = maskKey(proKeyReal);
  });
