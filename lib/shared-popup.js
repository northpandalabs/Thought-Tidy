...
function showResult(results, error) {
  ...
  // 3+ results: open in a new tab (extension) or a new window (desktop)
  if (normalized.length >= 3) {
    const isExtension = window.POPUP_SOURCE === "extension" && typeof browser !== "undefined" && typeof browser.tabs !== "undefined";
    const isDesktop   = window.POPUP_SOURCE === "desktop"   && typeof btcAPI !== "undefined" && typeof btcAPI.openResults === "function";
    if (isExtension || isDesktop) {
      browser.storage.local.set({ expandedResults: { results: normalized, timestamp: Date.now() } })
        .then(() => {
          if (isExtension) browser.tabs.create({ url: browser.runtime.getURL("popup/results.html") });
          else              btcAPI.openResults();
        });
      const notice = document.createElement("div");
      notice.className = "result-text";
      notice.style.cssText = "background:var(--accent-bg);border-color:var(--accent);text-align:center;max-height:none;";
      notice.textContent = `Opened ${normalized.length} suggestions in a new ${isDesktop ? "window" : "tab"} →`;
