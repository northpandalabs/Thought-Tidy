...
function rebuildVariantsSelect() {
  ...
  if (!sel) return;
  const isPro     = isProUnlocked(_settings);
  const actionVal = _actionOverride || document.getElementById("action-select")?.value;
  const ccBtn     = document.getElementById("clarity-check-btn");
  if (ccBtn) {
    const showCc = isPro && (_settings.showClarityCheckBtn !== false);
    ccBtn.style.display = showCc ? "" : "none";
    const badge = ccBtn.querySelector(".cc-badge");
  ...
  sel.style.display = "";
  const savedVal = parseInt(_settings.variants, 10) || 1;
  sel.innerHTML = "";
  for (let i = 1; i <= 4; i++) {
    const opt = document.createElement("option");
...
function rebuildActionDropdown() {
  ...
  const isPro      = isProUnlocked(_settings);
  const enabledActs = storedActs.filter(a => a.enabled && a.id !== "clarity-check");
  ...
  if (!sel._variantsWired) {
    ...
    sel.addEventListener("change", () => { _actionOverride = null; rebuildVariantsSelect(); });
  }
}
...
function initTextareaAutogrow() {
  ...
  const COLLAPSE_H  = 80;
  const SHOW_THRESH = 120;

  const collapseWrap = document.createElement("div");
  collapseWrap.style.cssText = "display:none; justify-content:flex-end; margin-top:2px;";
  const collapseBtn = document.createElement("button");
  collapseBtn.type        = "button";
  collapseBtn.className   = "ta-collapse-btn";
  collapseBtn.textContent = "↑ Collapse";
  collapseWrap.appendChild(collapseBtn);
  ta.after(collapseWrap);
  ...
  const resize = () => {
    ta.style.height = "auto";
    const natural   = Math.min(ta.scrollHeight, maxH());
    if (_collapsed && natural >= SHOW_THRESH) {
      ta.style.height    = COLLAPSE_H + "px";
      ta.style.overflowY = "auto";
    } else {
      if (_collapsed) { _collapsed = false; collapseBtn.textContent = "↑ Collapse"; }
      ta.style.height    = natural + "px";
      ta.style.overflowY = ta.scrollHeight > maxH() ? "auto" : "hidden";
    }
    collapseWrap.style.display = natural >= SHOW_THRESH ? "flex" : "none";
  };
...
function restoreContextAudience() {
  ...
  if (_settings.contextEnabled === false) {
    if (btn)   { btn.style.display = "none"; btn.textContent = "+ Add context"; btn.classList.remove("active"); }
...
function wireContextSheetHandlers() {
  document.getElementById("toggle-context-btn")?.addEventListener("click", () => {
    const sheet  = document.getElementById("context-sheet");
    const btn    = document.getElementById("toggle-context-btn");
    const runBtn = document.getElementById(window.RUN_BTN_ID || "run-btn");
    ...
    if (runBtn) runBtn.disabled = !isOpen;
  });
  document.getElementById("context-audience-select")?.addEventListener("change", () => {
    const runBtn = document.getElementById(window.RUN_BTN_ID || "run-btn");
...
function showResult(results, error) {
  ...
  if (normalized.length >= 2) {
    ...
    expandBtn.addEventListener("click", () => {
      ...
      const cols = parseInt(expandBtn.dataset.colCount, 10) || 2;
...
function showClarify(clarifyText, originalText, systemPrompt, meta) {
  ...
  const oldBtn = document.getElementById("clarify-submit-btn");
  if (!oldBtn) return;
  const btn = oldBtn.cloneNode(true);
  oldBtn.replaceWith(btn);
  const handler = async () => {
    const answers = sanitizeContent(answersEl.value);
    if (!answers) {
...
async function runProcess() {
  ...
  if (!hasProviders && !hasLegacy) {
    document.getElementById("result-area").style.display = "block";
    showResult(null, "No AI provider configured. Open Settings → add an API key, or set up Ollama for free local AI.");
    return;
  }
  const cps       = _settings.customPrompts || [];
  let   systemPrompt;
  ...
  const count  = actionVal === "fix-spelling" || actionVal === "clarity-check" || !isPro
    ? 1
    : Math.max(1, Math.min(4, parseInt(document.getElementById("variants-select")?.value, 10) || 1));
