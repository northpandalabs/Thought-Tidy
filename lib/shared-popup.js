...
function showResult(results, error) {
  ...
  // Desktop: 2+ results → toggle button; 3+ auto-expands into columns immediately
  if (window.POPUP_SOURCE === "desktop" && typeof btcAPI !== "undefined" && normalized.length >= 2) {
    const colCount = normalized.length;
    const resize = (n) => { if (typeof btcAPI.resizePopup === "function") btcAPI.resizePopup(n); };
    ...
    try {
      ...
      if (meta?.actionVal) {
        try {
          ...
          const cost        = estimateCost(r.usedModel || "", meta.initialText || originalText, [r.result]);
          historyFull.push({
            ...
            inputText: (meta.initialText || originalText).slice(0, 5000),
