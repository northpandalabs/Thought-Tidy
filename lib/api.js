...
async function callGitHubCopilot(token, model, prompt, text) {
  ...
  const res = await fetch("https://api.githubcopilot.com/chat/completions", {
    ...
    headers: {
      ...
      "Copilot-Integration-Id": "thought-tidy",
      "Editor-Version": "thought-tidy/1.0"
    },
    body: JSON.stringify({
      model,
      messages: [
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub Copilot: ${e.error?.message || res.statusText}`);
  }
  return (await res.json()).choices[0].message.content.trim();
}
...
const _PROVIDER_LABELS = { openai: "OpenAI", claude: "Claude", gemini: "Gemini", ollama: "Ollama", copilot: "GitHub Copilot" };
...
async function callAIWithFallback(configuredProviders, geminiModels, settings, prompt, text, { onStatusUpdate } = {}) {
  ...
  for (const p of providers) {
    ...
    if (p.id === "gemini") {
      ...
      try {
        const result = await callOllama(baseUrl, model, prompt, text);
      ...
      try {
        const result = await callGitHubCopilot(p.apiKey, model, prompt, text);
...
if (typeof module !== "undefined") {
  module.exports = { callOpenAI, callClaude, callGemini, callOllama, callGitHubCopilot, callAI, callAIWithFallback, isRetriable };
}
